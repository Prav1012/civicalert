...
export default function DevDashboard() {
  ...
  const fetchReports = async () => {
    ...
    setError('')
    try {
      const res = await axios.get('http://localhost:8000/reports/all')
      setReports(res.data)
    } catch {
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{
          ...
          marginBottom: '28px', borderBottom: '1px solid rgba(0,170,255,0.15)',
          paddingBottom: '16px'
        }}>
          <div>
        ...
        {/* States */}
        {loading && (
          <div style={{ textAlign:'center', padding:'60px', color:'rgba(0,170,255,0.5)', letterSpacing:'3px' }}>
            ⏳ LOADING REPORTS...
          </div>
        )}
        {error && (
          <div style={{ padding:'16px', border:'1px solid rgba(255,68,68,0.3)', borderRadius:'4px', color:'#ff4444', fontSize:'12px' }}>
            {error}
          </div>
        )}
        {!loading && !error && filtered.length === 0 && (
          <div style={{ textAlign:'center', padding:'60px', color:'rgba(255,255,255,0.2)', letterSpacing:'2px', fontSize:'12px' }}>
            📭 NO REPORTS FOUND
          </div>
        )}
    ...
    <span>🕐 {new Date(r.created_at).toLocaleString()}</span>
              ...
              {selected?.id === r.id && (
                <div style={{ marginTop:'14px', paddingTop:'14px', borderTop:'1px solid rgba(0,170,255,0.2)' }}>
                  <div style={{ fontSize:'9px', color:'rgba(0,170,255,0.6)', letterSpacing:'3px', marginBottom:'10px' }}>
                    ◈ FULL REPORT DATA
                  </div>
                  <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.4)', lineHeight:'2', fontFamily:'inherit' }}>
                    <div>🆔 REPORT ID: <span style={{color:'#00aaff'}}>#{r.id}</span></div>
                    <div>👤 ANON ID: <span style={{color:'#00aaff'}}>{r.anonymous_id?.slice(0,32)}...</span></div>
                    <div>⛓️ BLOCKCHAIN TX: <span style={{color:'#00aaff'}}>{r.blockchain_tx_hash?.slice(0,32)||'N/A'}</span></div>
                    <div>🖼️ IPFS HASH: <span style={{color:'#00aaff'}}>{r.photo_ipfs_hash||'No photo attached'}</span></div>
                    <div>📍 COORDINATES: <span style={{color:'#00aaff'}}>{r.latitude}, {r.longitude}</span></div>
                    <div>📝 FULL DESC: <span style={{color:'white'}}>{r.description}</span></div>
                  </div>
                </div>
              )}
            </div>
